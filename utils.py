...
#     if(i == len(headings)-1):
#       print(1)
#       print(web_html[web_html.index(0)])
#       try:
#        print(web_html[web_html.index(0)])
...
#       print(2)
#       try:
#        # print(fulltext(web_html[web_html.index(left)+len(left) : web_html.index(right)]))
#        # print(0)
...
def divideIntoQnA(web_html):
    '''
    Use: Divide the privacy policies into subheadings
    and return a dictionary of heading as a key, and data as value.
    ...
    for i in range(len(headings)):
        if i == len(headings)-1:
            try:
                # Using regular expressions to find content between headings
                content_between_headings = re.search(r'{}(.*?)(?={})'.format(headings[i], headings[i]), web_html, re.DOTALL)
                if content_between_headings:
                    qna[headings[i].strip()] = content_between_headings.group(1).strip()
            except:
                pass
        else:
            left = headings[i]
            right = headings[i+1]
            try:
                content_between_headings = re.search(r'{}(.*?){}'.format(left, right), web_html, re.DOTALL)
                if content_between_headings:
                    qna[headings[i].strip()] = content_between_headings.group(1).strip()
            except:
                pass
